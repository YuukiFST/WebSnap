...
class WebsiteDownloader:
    def __init__(self, url, output_dir, log_callback=None, page=None):
        self.url = url
    ...
    def _resolve_pending(self):
        ...
        self.log(f"📥 Baixando {len(urls)} recursos em paralelo...")
        results = self._batch_client.download_many(urls)

        for url, content in results.items():
            local_path = self._save_resource(url, content, '')
            if local_path:
                self.network_resources[url] = {
                    'body': content,
                    'content_type': '',
                }
    ...
    def process(self):
        if self._page is not None:
            return self._process_with_page()
        else:
            return self._process_full()
...
def zip_directory_to_memory(folder_path):
    buf = io.BytesIO()
